...
import {
  GetSearchPatientAction,
  clearSearchPatientState,
  DeletePatientAction,
...
export default function ItemList(props) {
  ...
  useEffect(() => {
    if (searchResponse != '' && searchResponse !== 'loading') {
      if (
        searchResponse.hasOwnProperty('data') &&
        searchResponse.data.hasOwnProperty('data')
      ) {
  ...
  const fetchMore = () => {
    if (initPager == 1 || totalItems < 16) {
      ...
      setrefreshBool(true);
      dispatch(GetSearchPatientAction(props.props, initPager));
    }
  };
