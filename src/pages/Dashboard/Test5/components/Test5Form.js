...
import data from '../../../../data/Test5Data';
...
export default function Test5() {
  ...
  const [result5, setResult5] = useState({value: ' '});
  ...
  useEffect(() => {
    if (singleResponse != '' || singleResponse != 'loading') {
      if (singleResponse.hasOwnProperty('data')) {
        ...
        dispatch(GetSingleTestAction(singleResponse.data.test_id, grade5,singleResponse.data.grade));
  ...
  const handleNext = () => {
    if (currentQuestionIndex == allQuestions.length - 1) {
      ...
      if (result5.value.includes(allQuestions[currentQuestionIndex]?.correct_option) ){
      ...
      if (pass && allQuestions[currentQuestionIndex]?.graded) {
        console.log('GradeEEEEEEEEEEEEEEEE previous: ', grade5);
        setGrade5(grade5 + 1);
        console.log('Grade51111111111111111111111111111111: ', grade5);
      }

      console.log('NAVIGATEEEEEEEE', grade5);
